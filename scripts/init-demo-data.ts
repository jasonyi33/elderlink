...
async function initializeDemo() {
  ...
  try {
    ...
    const verifyProfile = await fetch(`${WORKER_URL}/api/senior/mrs-chen`).then(r => r.json());
    const verifySentiment = await fetch(`${WORKER_URL}/api/sentiment/mrs-chen`).then(r => r.json());
    ...
    console.error('L Error initializing demo data:', error);
    process.exit(1);
  }
}
